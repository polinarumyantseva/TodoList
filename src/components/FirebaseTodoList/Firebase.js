...
export const FirebaseTodoList = () => {
	...
	const { addTodoItem, isCreating } = useRequestAddTodoItem(todoItemInputValue, setTodoItemInputValue);
	const { searchTodoItem, isSearching } = useRequestSearchTodoItem(setTodosList, todosList, todoItemInputValue);
	const { editItemId, editTodoItem, setValueForEdit } = useRequestUpdateTodoList();
	const { deleteTodoItem, isDeleting } = useRequestDeleteTodoItem();
		...
		<div className={styles.container}>
			...
			{isLoading ? (
				...
				<ul className={styles['todos-list']}>
					{Object.entries(todosList).map(([id, { title }]) => (
